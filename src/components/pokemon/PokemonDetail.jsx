import { colorByStat, pokemonColorType } from "../../constans/Pokemon";
...
const PokemonDetail = ({ pokemon }) => {
  return (
    <>
      ...
      <div className="overflow-y-auto px-4 pt-10 grid gap-2 content-start max-h-[75vh] hidden-scroll">
        ...
        <ul className="flex gap-2 justify-center">
          {pokemon?.types.map((type) => (
            <li
              className={`p-1 rounded-md px-2 text-white text-sm ${pokemonColorType[type] || "bg-slate-500"}`}
              key={type}
            >
              {type}
            </li>
